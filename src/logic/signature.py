...
class SignatureProcessor:
    def __init__(
        self, gui, app_config, setup_progress_cb, update_progress_cb, hide_progress_cb, excel_handler_class=None
    ):
        self.gui = gui
    ...
    def _apply_signature_schede(self, workbook, file_name):
        try:
            ...
            if cleaned_model in self.firma_processing_data:
                ...
                top_pos = max(
                    0, target_cell.Top - (offset_03cm if cleaned_model == "SCHEDAMANUTENZIONE" else offset_1cm)
                )
                left_pos = max(0, target_cell.Left - offset_1cm)
                ws.Shapes.AddPicture(
                    self.app_config.firma_image_path.get(), True, True, left_pos, top_pos, img_width, img_height
                )
                pdf_file_path = os.path.join(
                    self.app_config.firma_pdf_dir.get(), f"{os.path.splitext(file_name)[0]}.pdf"
                )
    ...
    def _apply_signature_preventivi(self, workbook, file_name):
        try:
            ...
            if ws is None:
                self.logger("Foglio 'Consuntivo' non trovato.", "WARNING")
                return
            ws.Activate()
            ...
            ws.Shapes.AddPicture(
                self.app_config.firma_image_path.get(), True, True, target_cell.Left, top_position, 150, 50
            )
