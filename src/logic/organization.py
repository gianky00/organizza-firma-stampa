    ...
    def run_organization_process(self, cancel_event):
        ...
        try:
            try:
                ...
                self.logger(
                    "L'operazione di organizzazione è stata interrotta per prevenire la perdita di dati.", "ERROR"
                )
            ...
            for i, fp in enumerate(excel_files):
                ...
                try:
                    ...
                    odc_v = next(
                        (
                            ws.Range(c).Value
                            for c in ["L50", "L45", "DB14", "DB17"]
                            if ws.Range(c).Value is not None and str(ws.Range(c).Value).strip() != ""
                        ),
                        None,
                    )
                    odc_s = (
                        str(int(odc_v))
                        if isinstance(odc_v, (int, float))
                        else (str(odc_v).strip() if isinstance(odc_v, str) else "")
                    )
        ...
        with ExcelHandler(self.logger) as excel:
            ...
            for i, folder_p in enumerate(folder_list):
                ...
                try:
                    ...
                    errors.append((os.path.basename(folder_p), f"Dettagli: {e_folder}"))
